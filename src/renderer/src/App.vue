...
import type { AppSettings, GuiExecutionReceipt, GuiAuditLogEntry } from './env';

type SettingsCategory = 'overview' | 'environment' | 'chat' | 'openclaw' | 'automation';
...
const view = ref<'home' | 'detecting' | 'result' | 'guide' | 'settings' | 'agent'>('home');
...
async function scrollAgentMessagesToBottom() {
  await nextTick();
  const el = agentMessagesContainer.value;
  if (!el) return;
...
const windowTitle = computed(() => {
  if (view.value === 'detecting') return '环境检测中';
  if (view.value === 'result') return '检测结果';
  if (view.value === 'guide') return '安装引导';
  if (view.value === 'settings') return '设置';
  if (view.value === 'agent') return 'OpenClaw 对话';
  return 'OpenClaw 安装助手';
});
...
<template>
  <div :class="['app', 'arco-theme-light', (view === 'settings' || view === 'agent') && 'app--wide']">
    <WindowTitleBar
      :title="windowTitle"
      :is-maximized="windowIsMaximized"
      @minimize="onWindowMinimize"
      @maximize="onWindowMaximizeToggle"
      @close="onWindowClose"
    />
    ...
    <header class="header header--ollama">
      <div class="header-brand">
        ...
        <a-typography-text v-if="view !== 'detecting'" type="secondary" class="subtitle">
          帮助你在本机快速安装与配置 OpenClaw
        </a-typography-text>
      </div>
      <div class="header-toolbar">
        <a-button v-if="view === 'result'" type="text" size="small" @click="onBack">返回</a-button>
        <a-button v-else-if="view === 'guide'" type="text" size="small" @click="view = 'result'">返回检测结果</a-button>
        <a-button v-else-if="view === 'settings'" type="text" size="small" @click="onBackFromSettings">
          {{ settingsWizardCompleted ? '返回主界面' : '返回' }}
        </a-button>
        <a-button
          v-if="view !== 'settings' && view !== 'detecting'"
          type="text"
    ...
    <main class="main">
      ...
      <InstallationGuideView
        v-else-if="view === 'guide' && guideReport"
        ...
        :install-daemon-running="installDaemonRunning"
        ...
        @install-nvm="doInstallNvm"
        @open-node-url="openNodeUrl"
        @install-node-nvm="doInstallNodeViaNvm"
        @install-openclaw="doInstallOpenClaw"
        @start-gateway="doStartGatewayInProcess"
        @install-daemon="doInstallDaemon"
        @open-config-dir="openConfigDir"
        @free-yes="onFreeModelYes"
        @free-no="onFreeModelNo"
        @ollama-serve="onOllamaServe"
        @write-provider="onWriteOllamaProvider"
        @enter-config="onEnterConfig()"
      />


      <!-- 阶段三：主界面（Agent 对话） -->
      <section v-else-if="view === 'agent'" class="agent-view">
        <div ref="agentMessagesContainer" class="agent-messages">
      ...
      <!-- 设置：左侧分类 + 右侧面板 -->
      <div v-else-if="view === 'settings'" class="settings-shell">
        <aside class="settings-nav" aria-label="设置分类">
          <div
            v-for="nav in settingsNavItems"
            :key="nav.key"
            class="settings-nav-item"
            :class="{ 'settings-nav-item--active': settingsCategory === nav.key }"
            role="button"
            tabindex="0"
            @click="onSettingsCategorySelect(nav.key)"
            @keydown.enter.prevent="onSettingsCategorySelect(nav.key)"
          >
            {{ nav.label }}
          </div>
        </aside>
        <div class="settings-main">
          <div class="settings-panel-scroll">
            <div v-show="settingsCategory === 'overview'" class="settings-panel">
            ...
            <div v-show="settingsCategory === 'environment'" class="settings-panel">
              ...
              <a-form layout="vertical" class="settings-form settings-form--panel">
                ...
                <a-form-item>
                  <a-button type="primary" @click="saveSettings">保存本页设置</a-button>
                </a-form-item>
              </a-form>
            </div>

            <div v-show="settingsCategory === 'chat'" class="settings-panel">
            ...
            <div v-show="settingsCategory === 'openclaw'" class="settings-panel">
              ...
              <a-form layout="vertical" class="config-form config-form-wide settings-form--panel">
                ...
                <a-form-item>
                  <a-space wrap>
                    <a-button type="primary" :loading="configSaveLoading" @click="saveOpenClawFormFromSettings">保存 OpenClaw 配置</a-button>
                    <a-button
                      v-if="!settingsWizardCompleted"
                      type="outline"
                    ...
                    <a-button @click="onBackFromOpenclawPanel">
                      {{ configReturnTo === 'agent' ? '返回主界面' : configReturnTo === 'settings' ? '返回概览' : '返回安装引导' }}
                    </a-button>
                  </a-space>
                </a-form-item>
              </a-form>
            ...
            <div v-show="settingsCategory === 'automation'" class="settings-panel">
              ...
              <a-form layout="vertical" class="settings-form settings-form--panel">
                ...
                <a-form-item label="筛选与导出">
                  ...
                  <div v-if="settingsGuiAuditFiltered.length" class="gui-audit-table-wrap">
                ...
                <a-form-item>
                  <a-button type="primary" :loading="configSaveLoading" @click="saveOpenClawFormFromSettings">保存 GUI 策略</a-button>
                </a-form-item>
              </a-form>
            </div>
          </div>
    ...
    <footer v-if="view !== 'agent'" class="footer">
